...
public class HighscoresDemo : MonoBehaviour, ITableViewDataSource
{
    ...
    private bool once;
    public void renove_login()
    {
        login.SetActive(false);
        database_main.second_time_login = true;
	...
	void Update ()
	{
        if (levecomplete.level_complete == true && once == false)
        {
            once = true;
            UpdateScore();
        }
	...
	private void OnInsertCompleted (IRestResponse<Highscore> response)
	{
		if (!response.IsError && response.StatusCode == HttpStatusCode.Created) {
            ...
            renove_login();
            PlayerPrefs.SetString("playerid",database_main.player_id);
            PlayerPrefs.SetString("username",database_main.playerusername);
			_score = item;
		} else {
	...
	private Highscore GetScore ()
	{
        ...
        database_main.playerusername = name;
		return highscore;
	}
	...
	private void DisplayScore (Highscore highscore)
	{
        ...
        database_main.player_id = highscore.id;
		UpdateUI ();
	}
